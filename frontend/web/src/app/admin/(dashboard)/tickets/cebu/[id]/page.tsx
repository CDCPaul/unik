...
import CebuPacificTicket from '@/components/tickets/CebuPacificTicket';
import CebuPacificTicketPDF from '@/components/tickets/CebuPacificTicketPDF';
...
export default function CebuPacificTicketDetailPage() {
  ...
  const handlePreviewTicket = (passengerIdx: number) => {
    ...
    import('react-dom/client').then((ReactDOM) => {
      import('react').then((React) => {
        ...
        root.render(
          React.createElement(CebuPacificTicket, { ticket, passenger })
        );
          ...
          previewWindow.document.write(`
            ...
            <html>
              <head>
                ...
                <title>Cebu Pacific Ticket Preview - ${passenger.lastName} ${passenger.firstName}</title>
                <style>
                  * {
  ...
  const loadTicket = async () => {
    ...
    try {
      ...
      if (data) {
        // Verify it's a Cebu Pacific ticket
        if (data.airline !== '5J') {
          alert('This page is for Cebu Pacific tickets only.');
  ...
  const handleGeneratePdf = async (passengerIndex?: number) => {
    ...
    try {
      if (typeof passengerIndex === 'number') {
        ...
        const filename = `5J_${ticket.reservationNumber}_${passenger.lastName}_${passenger.firstName}.pdf`;
        
        const pdfBlob = await generatePdfFromReactPDF(
          <CebuPacificTicketPDF ticket={ticket} passenger={passenger} />,
          filename
        );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <span className="px-3 py-1 text-sm rounded-lg bg-green-100 text-green-800 border border-green-200">
              Cebu Pacific (5J)
            </span>
            Ticket Details
          </h1>
              ...
              {passengers.map((passenger, idx) => (
                <div key={idx} className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg border border-slate-200 hover:bg-slate-100 transition-colors">
                  <input
                    ...
                    className="w-5 h-5 text-green-600 border-slate-300 rounded focus:ring-green-500 flex-shrink-0"
                  />
                      ...
                      {!isEditing && (
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-1 text-xs font-medium text-green-600 hover:text-green-700 hover:bg-green-50 rounded-lg transition-colors flex items-center gap-1 disabled:opacity-50"
                          >
                    ...
                    {isEditing ? (
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-slate-900">{passenger.type || 'Adult'}</p>
                        </div>
                      </div>
                    )}
                  </div>

...
import { generatePdfFromElement, downloadPdf } from '@/lib/utils/pdf';
import JejuAirTicket from '@/components/tickets/JejuAirTicket';
...
export default function JejuAirTicketDetailPage() {
  ...
  const handlePreviewTicket = (passengerIdx: number) => {
    ...
    import('react-dom/client').then((ReactDOM) => {
      import('react').then((React) => {
        ...
        root.render(
          React.createElement(JejuAirTicket, { ticket, passenger })
        );
          ...
          previewWindow.document.write(`
            ...
            <html>
              <head>
                ...
                <title>Jeju Air Ticket Preview - ${passenger.lastName} ${passenger.firstName}</title>
                <style>
                  * {
  ...
  const loadTicket = async () => {
    ...
    try {
      ...
      if (data) {
        // Verify it's a Jeju Air ticket
        if (data.airline !== 'JEJU') {
          alert('This page is for Jeju Air tickets only.');
  ...
  const handleGeneratePdf = async (passengerIndex?: number) => {
    ...
    try {
      if (typeof passengerIndex === 'number') {
        // Generate single PDF using html2canvas
        const element = ticketRefs.current[passengerIndex];
        if (!element) {
        ...
        const filename = `JEJU_${ticket.reservationNumber}_${passenger.lastName}_${passenger.firstName}.pdf`;
        
        const pdfBlob = await generatePdfFromElement(element, filename);
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <span className="px-3 py-1 text-sm rounded-lg bg-orange-100 text-orange-800 border border-orange-200">
              Jeju Air (JEJU)
            </span>
            Ticket Details
          </h1>
              ...
              {passengers.map((passenger, idx) => (
                <div key={idx} className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg border border-slate-200 hover:bg-slate-100 transition-colors">
                  <input
                    ...
                    className="w-5 h-5 text-orange-600 border-slate-300 rounded focus:ring-orange-500 flex-shrink-0"
                  />
                      ...
                      {!isEditing && (
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-1 text-xs font-medium text-orange-600 hover:text-orange-700 hover:bg-orange-50 rounded-lg transition-colors flex items-center gap-1 disabled:opacity-50"
                          >
            ...
            {!isEditing && (
              <div className="mt-6 flex justify-end">
                <button
                  ...
                  className="px-6 py-3 bg-orange-600 text-white rounded-lg font-medium hover:bg-orange-700 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isGeneratingPdf ? (
                    <>
      ...
      {/* Hidden Ticket Templates for PDF Generation */}
        ...
        <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
          {passengers.map((passenger, idx) => (
            <div 
              ...
              style={{ width: '896px', backgroundColor: '#ffffff', padding: '48px' }}
            >
              <JejuAirTicket ticket={ticket} passenger={passenger} />
            </div>
          ))}
        </div>
      )}
